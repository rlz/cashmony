...
export async function storeOperations(google: Google, operations: readonly Operation[], engine: Engine): Promise<void> {
    ...
    const rows = opsToGoogle(operations, engine)
...
export async function storeGoals(google: Google, goals: readonly Watch[], engine: Engine): Promise<void> {
    console.log('Store goals')

    await clearData(google, google.tabNames.goals)

    const rows = goalsToGoogle(goals, engine)
