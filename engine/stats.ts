...
export function hasOperation(engine: Engine, predicate: Predicate, timeSpan: HumanTimeSpan | null): boolean {
    const next = listOperations(engine, predicate, timeSpan).next()
...
export async function calcStats<T>(engine: Engine, predicate: Predicate, timeSpan: HumanTimeSpan | null, today: DateTime, reducers: T): Promise<InferReturnType<T>> {
    ...
    const [startDate, endDate] = (() => {
        ...
        return [engine.firstOp?.date ?? today, engine.lastOp?.date ?? today]
    })()
